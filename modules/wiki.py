...
def wikiSummary(keyword: str = None, dlu_key: str = Security(checkDluKey)):
    ...
    try:
        with sessions.Session() as s:
            search: Response = s.get(
                f"https://en.wikipedia.org/w/api.php?action=query&list=search&srsearch={keyword}&format=json"
            )
            search.raise_for_status()
            search: dict = search.json()
            if search.get("query").get("search"):
                title = search.get("query").get("search")[0].get("title")
                res = s.get(
                    f"https://en.m.wikipedia.org/api/rest_v1/page/summary/{title}"
                )
                res: dict = res.json()
